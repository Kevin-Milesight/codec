...
function _hexToBytes(hex){hex=String(hex).replace(/[^0-9a-fA-F]/g,"");if(hex.length%2)hex="0"+hex;var out=[];for(var i=0;i<hex.length;i+=2)out.push(parseInt(hex.substr(i,2),16));return out;}
function _encodeRaw(obj){var raw=obj&&(obj.raw_downlink!==undefined?obj.raw_downlink:obj);if(typeof raw!=="string"||!raw)return{bytes:[],fPort:1};var parts=raw.split(":");var hex=parts[0];var port=parts[1]?parseInt(parts[1],10):1;return{bytes:_hexToBytes(hex),fPort:port};}
function _atimFlatten(data){
  if(!data) return {};
  var out={};
  for(var k in data){
    var v=data[k];
    ...
    if(typeof v==="object"&&!Array.isArray(v)){
      if(v.type!==undefined){out[k]=v.type;continue;}
      if(v.value!==undefined){var val=v.value;out[k]=Array.isArray(val)?val[0]:val;continue;}
...
function _atimDecode(bytes,fPort){
  ...
  if(out.temperature_pir!==undefined){out.temp_sensor_voltage=out.temperature_pir;delete out.temperature_pir;}
  if(out.ouverture_boitier!==undefined){out.case_opened=(out.ouverture_boitier==="boitier ouvert")?1:0;delete out.ouverture_boitier;}
  if(out.capteur_dq!==undefined){
    var dqStr=out.capteur_dq;
    out.dq_sensor=(dqStr==="Active")?0:(dqStr==="Non utilisé")?255:1;
    delete out.capteur_dq;
  }
  if(out.motion_detection_count!==undefined){out.motion_detection=(out.motion_detection_count>0)?1:0;}
  if(out.type_trame!==undefined){out.motion_detection_alarm=(out.type_trame==="Trame alarme")?1:0;delete out.type_trame;}
    return out;
  }catch(e){return {};}
}
