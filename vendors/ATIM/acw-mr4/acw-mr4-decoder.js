...
function _atimDecode(bytes, fPort) {
  try {
    ...
    var d = r.data;
    var out = {};
    if (d.frame_type && d.frame_type.type !== undefined) out.frame_type = d.frame_type.type;
    if (d.tensionv && d.tensionv.value !== undefined) out.voltage_idle = d.tensionv.value;
    if (d.tensionc && d.tensionc.value !== undefined) out.voltage_tx = d.tensionc.value;
    if (d.entree && Array.isArray(d.entree.value)) {
      for (var j = 0; j < Math.min(16, d.entree.value.length); j++) {
        var ev = d.entree.value[j];
        if (Array.isArray(ev) && ev.length > 0) out['input_' + (j + 1)] = ev[0];
        else if (typeof ev === 'number') out['input_' + (j + 1)] = ev;
        else if (typeof ev === 'string') out['input_' + (j + 1)] = ev;
      }
    }
    for (var n = 0; n <= 3; n++) {
      var ck = 'compte' + n;
      if (d[ck] && Array.isArray(d[ck].value) && d[ck].value.length > 0)
        out[['first_index','second_index','third_index','fourth_index'][n]] = d[ck].value[0];
    }
    if (d.distance && Array.isArray(d.distance.value) && d.distance.value.length > 0)
      out.distance = d.distance.value[0];
    if (d.temperature0 && Array.isArray(d.temperature0.value) && d.temperature0.value.length > 0)
      out.temperature = d.temperature0.value[0];
    if (d.humidity0 && Array.isArray(d.humidity0.value) && d.humidity0.value.length > 0)
      out.humidity = d.humidity0.value[0];
    if (d.CO2 && Array.isArray(d.CO2.value) && d.CO2.value.length > 0)
      out.co2 = d.CO2.value[0];
    if (d.COV && Array.isArray(d.COV.value) && d.COV.value.length > 0)
      out.cov = d.COV.value[0];
    if (d.pression_differentielle && Array.isArray(d.pression_differentielle.value) && d.pression_differentielle.value.length > 0)
      out.pression_differentielle = d.pression_differentielle.value[0];
    if (d.voltage && d.voltage.value !== undefined) out.voltage = d.voltage.value;
    if (d.current && d.current.value !== undefined) out.current = d.current.value;
    if (d.logic_level && Array.isArray(d.logic_level.value)) {
      if (d.logic_level.value.length > 0) out.input_1 = d.logic_level.value[0];
      if (d.logic_level.value.length > 1) out.input_2 = d.logic_level.value[1];
    }
    if (d.wirecut && d.wirecut.value !== undefined) out.wire_cut = d.wirecut.value;
    if (d.compteur1 && d.compteur1.value !== undefined) out.first_index = d.compteur1.value;
    if (d.compteur2 && d.compteur2.value !== undefined) out.second_index = d.compteur2.value;
    if (d.compteur_pir && d.compteur_pir.value !== undefined) out.motion_detection_count = d.compteur_pir.value;
    if (d.temperature_pir && d.temperature_pir.value !== undefined) out.temp_sensor_voltage = d.temperature_pir.value;
    if (d.ouverture_boitier && d.ouverture_boitier.value !== undefined) {
      out.case_opened = (d.ouverture_boitier.value === 'boitier ouvert') ? 1 : 0;
    }
    if (d.capteur_dq && d.capteur_dq.value !== undefined) {
      if (d.capteur_dq.value === 'Active') out.dq_sensor = 1;
      else if (d.capteur_dq.value === 'Non utilisé') out.dq_sensor = 255;
      else out.dq_sensor = 0;
    }
    if (d.type_trame && d.type_trame.value !== undefined) out.type_trame = d.type_trame.value;
    return out;
  } catch (e) { return {}; }
}
function Decode(fPort, bytes) { return _atimDecode(bytes, fPort); }
function Decoder(bytes, port) { return _atimDecode(bytes, port); }
if (typeof module !== 'undefined') {
  module.exports = {
    decodeUplink: function(i) { return { data: _atimDecode(i.bytes, i.fPort) }; },
    Decode: Decode,
    Decoder: Decoder
  };
}
