...
function _atimDecode(bytes, fPort) {
  try {
    ...
    if (d.tensionv && d.tensionv.value !== undefined) out.tension_alim_idle = d.tensionv.value;
    if (d.tensionc && d.tensionc.value !== undefined) out.tension_alim_tx = d.tensionc.value;
    if (d.entree && Array.isArray(d.entree.value)) {
      for (var j = 0; j < Math.min(16, d.entree.value.length); j++) {
        var ev = d.entree.value[j];
        if (Array.isArray(ev) && ev.length > 0) out['entree_' + (j + 1)] = ev[0];
        else if (typeof ev === 'number') out['entree_' + (j + 1)] = ev;
        else if (typeof ev === 'string') out['entree_' + (j + 1)] = ev;
      }
    }
    for (var n = 0; n <= 3; n++) {
      var ck = 'compte' + n;
      if (d[ck] && Array.isArray(d[ck].value) && d[ck].value.length > 0)
        out['compteur_' + (n + 1)] = d[ck].value[0];
    }
    ...
    if (d.logic_level && Array.isArray(d.logic_level.value)) {
      if (d.logic_level.value.length > 0) out.entree_1 = d.logic_level.value[0];
      if (d.logic_level.value.length > 1) out.entree_2 = d.logic_level.value[1];
    }
    if (d.wirecut && d.wirecut.value !== undefined) out.wire_cut = d.wirecut.value;
    if (d.compteur1 && d.compteur1.value !== undefined) out.compteur_1 = d.compteur1.value;
    if (d.compteur2 && d.compteur2.value !== undefined) out.compteur_2 = d.compteur2.value;
