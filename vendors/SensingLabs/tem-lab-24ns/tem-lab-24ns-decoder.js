// Sensing Labs SenlabT (SENLAB T - TEM-LAB-24NS) LoRaWAN payload decoder
...
function decodeDatalog(bytes, data) {
  ...
  // SenlabT 双探头（0x0C）：每 log = 温度(1B) + 温度2(1B)，单位 1/16 °C
  if (bytes.length >= 6) { data.temperature = bytes[5] / 16; }
  if (bytes.length >= 7) { data.temperature2 = bytes[6] / 16; }
}
