// Sensing Labs SenlabP (SENLAB P - PIR-LAB-51NS) LoRaWAN payload decoder
...
var MODEL_SLUG = "pir-lab-51ns";
var MODEL_FAMILY = "SenlabP";
...
function decodeDatalog(bytes, data) {
  ...
  // SenlabP：detection_count（人次计数），datalog 帧取尾部 2B 为最新 log（MSB first）
  if (bytes.length >= 7) {
    data.detection_count = u16be(bytes, bytes.length - 2);
  }
}
