// Sensing Labs SenlabH (SENLAB H - THY-LAB-41NS) LoRaWAN payload decoder
...
var MODEL_SLUG = "thy-lab-41ns";
var MODEL_FAMILY = "SenlabH";
var DATALOG_PREFIX = 0x03;
...
function decodeDatalog(bytes, data) {
  ...
  // SenlabH：每 log = 温度(1B /16 °C) + 湿度(1B %)
  if (bytes.length >= 6) { data.temperature = bytes[5] / 16; }
  if (bytes.length >= 7) { data.humidity = bytes[6]; }
}
