        ...
        switch (deviceType) {
            case EN302: /* TX T&H AMB  600-021*/
                ...
                addAlarmStateField(template,
                    alarmParser(payload.substring(28, 32), deviceType));
            ...
            case EN314: /* TX TEMP CONT2 MP 600-233 */
                ...
                addAlarmStateField(template,
                    alarmParser(payload.substring(20, 24), deviceType));
            ...
            case EN307: /*TX 4/20 mA 600-035 */
                ...
                addAlarmStateField(template,
                    alarmParser(payload.substring(16, 20), deviceType));
            ...
            case EN319:/* TX CONTACT 600-039*/
                ...
                addAlarmStateField(template,
                    alarmParser(payload.substring(36, 40), deviceType));
...
function decodeUplink(input) {
  ...
  var result = _origDecodeUplink(input);
  ...
  if (vals.Humidity      !== undefined) out.humidity      = vals.Humidity.value;
  if (vals.voc           !== undefined) out.voc           = vals.voc.value;
  if (vals.co2           !== undefined) out.co2           = vals.co2.value;
  if (vals.temperature_1 !== undefined) out.temperature_1 = vals.temperature_1.value;
  if (vals.temperature_2 !== undefined) out.temperature_2 = vals.temperature_2.value;
  if (vals.current       !== undefined) out.current       = vals.current.value;
  if (vals.pulse_ch1     !== undefined) out.pulse_ch1     = vals.pulse_ch1.value;
  if (vals.pulse_ch2     !== undefined) out.pulse_ch2     = vals.pulse_ch2.value;
  if (vals.pulse_oc      !== undefined) out.pulse_oc      = vals.pulse_oc.value;

  // flatten states
  if (states.battery  !== undefined) out.battery  = states.battery;
  if (states.msg_type !== undefined) out.msg_type = states.msg_type;
  if (states.status && states.status.msg_type !== undefined) out.msg_type = states.status.msg_type;
