...
function milesightDeviceEncode(payload) {
    ...
    if ("valve_control_settings" in payload) {
        encoded = encoded.concat(setValveControlSettings(payload.valve_control_settings));
    }
    if ("fan_control_settings" in payload) {
        encoded = encoded.concat(setFanControlSettings(payload.fan_control_settings));
    }
 ...
 * @param {number} valve_interface_settings.mode values: (0: four_pipe_two_wire, 1: two_pipe_two_wire, 2: two_pipe_three_wire)
 * @param {object} valve_interface_settings.four_pipe_two_wire
 * @param {object} valve_interface_settings.two_pipe_two_wire
 * @param {object} valve_interface_settings.two_pipe_three_wire
 */
function setValveInterfaceSettings(valve_interface_settings) {
    ...
    switch (mode_value) {
        case 0x00: // four_pipe_two_wire
            data = data.concat(setFourPipeTwoWireConfig(valve_interface_settings.four_pipe_two_wire));
            break;
        case 0x01: // two_pipe_two_wire
            data = data.concat(setTwoPipeTwoWireConfig(valve_interface_settings.two_pipe_two_wire));
            break;
        case 0x02: // two_pipe_three_wire
            data = data.concat(setTwoPipeThreeWireConfig(valve_interface_settings.two_pipe_three_wire));
            break;
    }
    return data;
}
...
/**
 * @param {object} four_pipe_two_wire
 * @param {number} four_pipe_two_wire.cooling_valve values: (0: valve_1, 1: valve_2)
 * @param {number} four_pipe_two_wire.heating_valve values: (0: valve_1, 1: valve_2)
 */
function setFourPipeTwoWireConfig(four_pipe_two_wire) {
    var cooling_valve = four_pipe_two_wire.cooling_valve;
    var heating_valve = four_pipe_two_wire.heating_valve;

    var valve_map = { 0: "valve_1", 1: "valve_2" }
    var valve_values = getValues(valve_map);
    if (valve_values.indexOf(cooling_valve) === -1) {
        throw new Error("four_pipe_two_wire.cooling_valve must be one of " + valve_values.join(", "));
    }
    if (valve_values.indexOf(heating_valve) === -1) {
        throw new Error("four_pipe_two_wire.heating_valve must be one of " + valve_values.join(", "));
    }

    var buffer = new Buffer(4);
    buffer.writeUInt8(0x7c);
    buffer.writeUInt8(0x00); // four_pipe_two_wire sub-command
...
/**
 * @param {object} two_pipe_two_wire
 * @param {number} two_pipe_two_wire.valve values: (0: valve_1, 1: valve_2)
 */
function setTwoPipeTwoWireConfig(two_pipe_two_wire) {
    var valve = two_pipe_two_wire.valve;

    var valve_map = { 0: "valve_1", 1: "valve_2" }
    var valve_values = getValues(valve_map);
    if (valve_values.indexOf(valve) === -1) {
        throw new Error("two_pipe_two_wire.valve must be one of " + valve_values.join(", "));
    }

    var buffer = new Buffer(3);
    buffer.writeUInt8(0x7c);
    buffer.writeUInt8(0x01); // two_pipe_two_wire sub-command
    buffer.writeUInt8(getValue(valve_map, valve));
    return buffer.toBytes();
}

/**
 * 
 * @param {object} two_pipe_three_wire
 * @param {number} two_pipe_three_wire.no_valve values: (0: valve_1, 1: valve_2)
 * @param {number} two_pipe_three_wire.nc_valve values: (0: valve_1, 1: valve_2)
 */
function setTwoPipeThreeWireConfig(two_pipe_three_wire) {
    var no_valve = two_pipe_three_wire.no_valve;
    var nc_valve = two_pipe_three_wire.nc_valve;

    var valve_map = { 0: "valve_1", 1: "valve_2" }
    var valve_values = getValues(valve_map);
    if (valve_values.indexOf(no_valve) === -1) {
        throw new Error("two_pipe_three_wire.no_valve must be one of " + valve_values.join(", "));
    }
    if (valve_values.indexOf(nc_valve) === -1) {
        throw new Error("two_pipe_three_wire.nc_valve must be one of " + valve_values.join(", "));
    }

    var buffer = new Buffer(4);
    buffer.writeUInt8(0x7c);
    buffer.writeUInt8(0x02); // two_pipe_three_wire sub-command
    buffer.writeUInt8(getValue(valve_map, no_valve));
    buffer.writeUInt8(getValue(valve_map, nc_valve));
    return buffer.toBytes();
}
