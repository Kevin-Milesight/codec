...
function decode(bytes, fPort) {
  ...
  function _voltVal(v){ if(typeof v==='number') return v; if(typeof v==='string'){ var m=v.match(/[\d.]+/); return m?parseFloat(m[0]):null; } return null; }
  function _voltLevel(v){ if(typeof v==='string'&&v.indexOf('low')>=0) return 'low'; return v==null?'':'ok'; }

  if (d.Volt!==undefined){ out.battery_voltage=_voltVal(d.Volt); out.battery=_voltLevel(d.Volt); }
  function _v(x){return x==='NoSensor'?null:x;}
  if (d.Temperature!==undefined) out.temperature=_v(d.Temperature);
  if (d.Humidity!==undefined) out.humidity=_v(d.Humidity);
  if (d.CO2!==undefined) out.co2_concentration=_v(d.CO2);
  if (d.PH!==undefined) out.ph=_v(d.PH);
  if (d.TempPH!==undefined) out.temperaturewithph=_v(d.TempPH);
  if (d.ORP!==undefined) out.orp=_v(d.ORP);
  if (d.NTU!==undefined) out.ntu=_v(d.NTU);
  if (d.TempNTU!==undefined) out.temperaturewithntu=_v(d.TempNTU);
  if (d.EC5SoildHumi!==undefined) out.ec5soilhumidity=_v(d.EC5SoildHumi);
  if (d.SoildHumi5TE!==undefined) out['5tesoilhumidity']=_v(d.SoildHumi5TE);
  if (d.SoildTemp5TE!==undefined) out['5tesoiltemp']=_v(d.SoildTemp5TE);
  if (d.WaterLevel!==undefined) out.waterlevel=_v(d.WaterLevel);
  if (d.PM1_0!==undefined) out.pm_1=_v(d.PM1_0);
  if (d.PM2_5!==undefined) out.pm_2_5=_v(d.PM2_5);
  if (d.PM10!==undefined) out.pm_10=_v(d.PM10);
  if (d.Cmd!==undefined) out.cmd=d.Cmd;
  return out;
}
