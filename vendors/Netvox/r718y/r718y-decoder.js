function getCmdId(inputcmd){
  var cmdIDs = {
    0x01: "ConfigReportReq",
    0x81: "ConfigReportRsp",
    0x02: "ReadConfigReportReq",
    0x82: "ReadConfigReportRsp"
  };
  return cmdIDs[inputcmd];
}

function getDeviceName(dev){
  var deviceName = {
    0xAC: "R718Y"
  };
  return deviceName[dev];
}

function getDeviceType(devName){
  if (devName == "R718Y")
	  return 0xAC;
}

function padLeft(str, len) {
    str = '' + str;
    if (str.length >= len) {
        return str;
    } else {
        return padLeft("0" + str, len);
    }
}

function decodeUplink(input) {
  var data = {};
  switch (input.fPort) {
    case 6:
		if (input.bytes[2] === 0x00)
		{
			data.Device = getDeviceName(input.bytes[1]);
			data.SWver =  input.bytes[3]/10;
			data.HWver =  input.bytes[4];
			data.Datecode = padLeft(input.bytes[5].toString(16), 2) + padLeft(input.bytes[6].toString(16), 2) + padLeft(input.bytes[7].toString(16), 2) + padLeft(input.bytes[8].toString(16), 2);
			
			return {
				data: data,
			};
		}

		if (input.bytes[3] & 0x80)
		{
			var tmp_v = input.bytes[3] & 0x7F;
			data.Volt = (tmp_v / 10).toString() + '(low battery)';
		}
		else
			data.Volt = input.bytes[3]/10;

		data.Device = getDeviceName(input.bytes[1]);
		if (input.bytes[4] & 0x80)
		{
			var tmpP = (input.bytes[4]<<8 | input.bytes[5]);
			data.DiffPressure = (0x10000 - tmpP)/10 * -1;
		}
		else
			data.DiffPressure = (input.bytes[4]<<8 | input.bytes[5])/10;
		
		if (input.bytes[6] & 0x80)
		{
			var tmpT = (input.bytes[6]<<8 | input.bytes[7]);
			data.Temp = (0x10000 - tmpT)/10 * -1;
		}
		else
			data.Temp = (input.bytes[6]<<8 | input.bytes[7])/10;

		break;
		
	case 7:
		data.Device = getDeviceName(input.bytes[1]);
		if (input.bytes[0] === 0x81)
		{
			data.Cmd = getCmdId(input.bytes[0]);
			data.Status = (input.bytes[2] === 0x00) ? 'Success' : 'Failure';
		}
		else if (input.bytes[0] === 0x82)
		{
			data.Cmd = getCmdId(input.bytes[0]);
			data.MinTime = (input.bytes[2]<<8 | input.bytes[3]);
			data.MaxTime = (input.bytes[4]<<8 | input.bytes[5]);
			data.BatteryChange = input.bytes[6]/10;
			data.DiffPressureChange = (input.bytes[7]<<8 | input.bytes[8])/10;
			data.TempChange = (input.bytes[9]<<8 | input.bytes[10])/10;
			
		}
		break;

	default:
      return {
        errors: ['unknown FPort'],
      };
	  
    }
          
	  
	 return {
		data: data,
	};
 }
  
function encodeDownlink(input) {
  var ret = [];
  var devid;
 
  devid = getDeviceType(input.data.Device);

  if (input.data.Cmd == getCmdId(0x01))
  {
	  var mint = input.data.MinTime;
	  var maxt = input.data.MaxTime;
	  var batteryChg = input.data.BatteryChange * 10;
	  var pressureChg = input.data.DiffPressureChange * 10;
	  var tempChg = input.data.TempChange * 10;

	  ret = ret.concat(0x01, devid, (mint >> 8), (mint & 0xFF), (maxt >> 8), (maxt & 0xFF), batteryChg, (pressureChg >> 8), (pressureChg & 0xFF), (tempChg >> 8), (tempChg & 0xFF));
  }
  else if (input.data.Cmd == getCmdId(0x02))
  {
	  ret = ret.concat(0x02, devid, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00);
  }  
  
  return {
    fPort: 7,
    bytes: ret
  };
}  
  
function decodeDownlink(input) {
  var data = {};
  switch (input.fPort) {
    case 7:
		data.Device = getDeviceName(input.bytes[1]);
		if (input.bytes[0] === 0x01)
		{
			data.Cmd = getCmdId(input.bytes[0]);
			data.MinTime = (input.bytes[2]<<8 | input.bytes[3]);
			data.MaxTime = (input.bytes[4]<<8 | input.bytes[5]);
			data.BatteryChange = input.bytes[6]/10;
			data.DiffPressureChange = (input.bytes[7]<<8 | input.bytes[8])/10;
			data.TempChange = (input.bytes[9]<<8 | input.bytes[10])/10;
		}
		else if (input.bytes[0] === 0x02)
		{
			data.Cmd = getCmdId(input.bytes[0]);
		}
		break;
		
    default:
      return {
        errors: ['invalid FPort'],
      };
  }
  
  return {
		data: data,
	};
}


function decode(bytes, fPort) {
  var input = { bytes: bytes, fPort: fPort };
  var res = _netvoxOrigDecodeUplink(input);
  if (!res || !res.data) return {};
  var d = res.data;
  var out = {};

  function _voltVal(v){ if(typeof v==='number') return v; if(typeof v==='string'){ var m=v.match(/[\d.]+/); return m?parseFloat(m[0]):null; } return null; }
  function _voltLevel(v){ if(typeof v==='string'&&v.indexOf('low')>=0) return 'low'; return v==null?'':'ok'; }

  if (d.Volt!==undefined){ out.battery_voltage=_voltVal(d.Volt); out.battery=_voltLevel(d.Volt); }
  if (d.DiffPressure!==undefined) out.different_pressure=d.DiffPressure;
  if (d.Temp!==undefined) out.temperature1=d.Temp;
  if (d.Cmd!==undefined) out.cmd=d.Cmd;
  return out;
}

function Decode(fPort, bytes) { return decode(bytes, fPort); }
function Decoder(bytes, port) { return decode(bytes, port); }
if (typeof module !== 'undefined') { module.exports = { decodeUplink: function(i){return {data: decode(i.bytes, i.fPort)};}, Decode: Decode, Decoder: Decoder }; }

var _netvoxOrigDecodeUplink = decodeUplink;
decodeUplink = function(input) { return { data: decode(input.bytes, input.fPort) }; };
